...
### Cross-flow `updateExecution`

When `flowAlias` differs from the handle's own alias, the handle resolves
and verifies that target flow before mutation. A missing target raises an
`AuthenticationFlowNotFoundError` (with `realmName` and `alias` fields) and
