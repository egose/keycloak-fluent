...
When `data.config.jwksUrl` is provided, `useJwksUrl` is normalized from that value: empty or whitespace-only strings set `useJwksUrl` to `'false'`, and non-empty strings set it to `'true'`. When `jwksUrl` is omitted, `create()` keeps the default `useJwksUrl: 'false'`, while `update()` and the update branch of `ensure()` preserve the identity provider's existing `useJwksUrl` setting.
...
Lists mappers configured on the identity provider.
...
Lists the mapper types available for the identity provider.
...
Returns an `IdentityProviderMapperHandle` scoped to the current identity provider.
